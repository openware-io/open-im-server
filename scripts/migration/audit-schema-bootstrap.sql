...
--   * 顺序颠倒 → Flyway 会在 open_saas 里建出分区表（与旧表同名，迁移会失败或写错库）。
--
...
--     （ACK 的 im_user 对 open_saas.* 已是全量授权）。
--   * 其它环境/原生 MySQL：同样手工执行一次。
--
-- 授权口径：
--   * open_audit.*：审计服务自己的库，全量权限（Flyway 建表、保留任务读分区元数据都需要）；
--   * open_saas.tnt_tenant：审计列表要回填租户名称（既有实现直接读这张表），只给只读；
--   * open_saas.saa_admin_account：审计列表/详情要按 operator_id 回填操作人姓名与登录名，只给只读。
--
-- 用法（可重复执行）：
...
GRANT SELECT ON `open_saas`.`tnt_tenant` TO '__DB_USERNAME__'@'%';
-- 审计列表/详情按 operator_id 回填操作人姓名与登录名（关联键是 saa_admin_account.platform_account_id）。
-- 同样是跨 schema 只读，只读这两张表，不新开其它读权限。
GRANT SELECT ON `open_saas`.`saa_admin_account` TO '__DB_USERNAME__'@'%';
