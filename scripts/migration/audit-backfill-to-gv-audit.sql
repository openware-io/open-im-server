...
ON DUPLICATE KEY UPDATE open_audit.iam_audit_log.id = open_audit.iam_audit_log.id;
...
WHERE COALESCE(occurred_at, created_at) >= __PERIOD_START__
  AND COALESCE(occurred_at, created_at) <  __PERIOD_END__
UNION ALL
SELECT 'open_audit(新)' AS source, COUNT(*) AS rows_in_period
FROM open_audit.iam_audit_log
...
FROM open_audit.iam_audit_log PARTITION (pmin)
UNION ALL
SELECT 'pmax' AS partition_name, COUNT(*) AS rows_in_partition
FROM open_audit.iam_audit_log PARTITION (pmax);
...
-- 因此回退前的写入窗口应记录在变更单上，必要时按 id 从 open_audit 反向补回旧表。
