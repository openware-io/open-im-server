package io.openware.platform.tenant.handler;
...
/**
 * 租户服务统一异常出口：把业务/权限/上下文异常映射为稳定 JSON（{code, message}）。
 *
 * <p>此前本服务没有 @RestControllerAdvice，controller 抛出的 ApiException（如
 * 403 TENANT_SCOPE_DENIED、403 PERMISSION_DENIED、401 SAAS_CONTEXT_REQUIRED）
 * 会退化成框架默认 500 `{"status":500,"error":"Internal Server Error"}`，
 * 前端只能拿到无法处理的错误页，也看不到真正的拒绝原因。
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleDataAccess(DataAccessException ex) {
        ...
        log.error("Unhandled tenant-service data access exception", ex);
        Map<String, Object> body = new LinkedHashMap<>();
    ...
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex, HttpServletRequest request) {
        log.error("Unhandled tenant-service exception, uri={}",
