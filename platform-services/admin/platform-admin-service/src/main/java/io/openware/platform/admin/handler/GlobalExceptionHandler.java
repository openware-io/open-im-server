package io.openware.platform.admin.handler;
...
/**
 * 平台后台统一异常出口：业务异常 + Spring MVC 框架 4xx 异常 + 500 兜底，全部返回 {code, message}。
 *
 * <p>此前本 advice 只映射 {@link ApiException}：任何框架异常（未知路径、方法不支持、缺请求头、
 * 请求体格式错误）都会退化成 Spring 默认错误体 {@code {"timestamp","status","error","path"}}，
 * 前端既拿不到业务 {@code code}，也无法按状态码区分「谁的问题」。因此这里显式声明框架异常，
 * 避免它们被 {@code Exception} 兜底截获成 500，并保留 404/405/415/400 的真实语义。
 */
...
public class GlobalExceptionHandler {
    ...
    /** 缺少必填请求头（如 Idempotency-Key）：既不能落 500，也不能漏出 Spring 默认错误体。 */
    ...
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex, HttpServletRequest request) {
        log.error("Unhandled admin-service exception, uri={}",
