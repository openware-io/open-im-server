...
public class AdminTenantContextTokenSigner {
  ...
  public String sign(long accountId, long tenantId, Long organizationId, Long storeId,
    ...
    Date now = new Date();
    return Jwts.builder().subject(String.valueOf(accountId)).issuer("gv-saas-identity")
  ...
  public String signPlatform(long accountId, Duration ttl) {
    ...
    return Jwts.builder().subject(String.valueOf(accountId)).issuer("gv-saas-identity")
        ...
        .claim("scopeType", com.gvchat.infrastructure.tenant.TenantContext.SCOPE_PLATFORM)
   ...
   */
  public com.gvchat.infrastructure.tenant.TenantContext verify(String token) {
    ...
    return new com.gvchat.infrastructure.tenant.TenantContext(tenant.longValue(),
  ...
  private Claims parseContextClaims(String token) {
    try {
      return Jwts.parser().verifyWith(secretKey).requireIssuer("gv-saas-identity")
          .requireAudience("gv-saas-services").build().parseSignedClaims(token).getPayload();
