package io.openware.platform.customer.handler;
...
/**
 * 会员/储值服务统一异常出口：业务异常 + Spring MVC 框架 4xx 异常 + 500 兜底，全部返回 {code, message}。
 *
 * <p>此前本 advice 只映射 {@link ApiException}：非业务异常一律落 Spring 默认错误体
 * {@code {"timestamp","status","error","path"}}，前端只能按状态码兜底；框架异常（未知路径、
 * 方法不支持、缺请求头）也无法区分 4xx/5xx。这里补齐显式映射与兜底日志。
 */
...
public class GlobalExceptionHandler {
    ...
    /** 路径/查询参数类型不匹配（如 customerId 传了非数字）。 */
    ...
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex, HttpServletRequest request) {
        log.error("Unhandled customer-service exception, uri={}",
