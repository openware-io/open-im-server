...
class UnitTimeFeeCalculatorTest {
  ...
  void roundingDirectionIsApplied() {
    assertEquals(PRICE, UnitTimeFeeCalculator.calculateTotal(
        33, INC, PRICE, io.openware.platform.order.domain.ktv.model.KtvRoundingDirection.ROUND_UP));
    assertEquals(0L, UnitTimeFeeCalculator.calculateTotal(
        33, INC, PRICE, io.openware.platform.order.domain.ktv.model.KtvRoundingDirection.CONSUMER_FAVOR));
  }
}
