...
部署脚本从未提交的仓库 `.env` 文件创建 `gv-im-env`，并从发布清单按 ACR `image@sha256:digest` 拉取 SaaS 候选镜像后导入专属 Kind 节点；Kind 不重新构建这些候选镜像。应用清单和 Windows 可访问的 NodePort 代理仍由本地脚本管理。默认集群名为 `gv-im-local`，上下文为 `kind-gv-im-local`。
...
本地 Docker 业务镜像按服务仓库最多保留最近两个版本。清理只针对本地镜像标签，必须先排除容器引用中的镜像；不删除 ACR 远程制品。`gv-im/rocketmq:5.3.1-local` 等规范指定的基础设施镜像按运行依赖保留。
