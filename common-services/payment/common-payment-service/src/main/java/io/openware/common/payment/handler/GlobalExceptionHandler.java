package io.openware.common.payment.handler;
...
/**
 * 支付服务统一异常出口：业务异常 + Spring MVC 框架 4xx 异常 + 500 兜底，全部返回 {code, message}。
 *
 * <p>业务码按对应 HTTP 状态返回：组合收款余额不足(LEDGER_INSUFFICIENT)、渠道未启用
 * (PAYMENT_CHANNEL_DISABLED)、幂等键缺失(IDEMPOTENCY_KEY_REQUIRED) 等。此前本 advice 只映射
 * {@link ApiException}，框架异常与未知异常会退化成 Spring 默认错误体且没有日志；这里补齐。
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception ex, HttpServletRequest request) {
        log.error("Unhandled payment-service exception, uri={}",
