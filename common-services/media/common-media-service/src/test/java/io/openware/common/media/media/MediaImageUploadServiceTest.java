...
class MediaImageUploadServiceTest {
  ...
  void uploadsImageIntoNeutralPublicBucketWithBizAndScopeKey() {
    ...
    assertThat(uploaded.bucket()).isEqualTo("open-media-public");
    ...
    assertThat(uploaded.url()).isEqualTo("/api/v1/media-public/open-media-public/" + uploaded.objectKey());
    verify(storage).ensurePublicReadBucket("open-media-public");
    // 缓存与内容类型写进对象元数据：网关只转发，响应头必须由对象存储回放，否则去掉入口反代后缓存语义丢失。
    verify(storage).putObject(eq("open-media-public"), eq(uploaded.objectKey()), eq("image/png"),
  ...
  void fallsBackToGatewayApiPrefixWhenConfiguredPrefixIsBlank() {
    MediaImageUploadService defaulted = new MediaImageUploadService(propertiesWith("open-media-public", "  "), storage);

    assertThat(defaulted.publicUrl("open-media-public", "saas/t1/202609/x.png"))
        .isEqualTo("/api/v1/media-public/open-media-public/saas/t1/202609/x.png");
  }
  ...
  private static MediaProperties properties() {
    return propertiesWith("open-media-public", "/api/v1/media-public");
  }
