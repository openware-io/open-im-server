package io.openware.common.media.handler;
...
/**
 * 媒体服务统一异常出口：把业务/框架异常映射为稳定 JSON（{code, message}）。
 *
 * <p>此前兜底分支没有日志、没有 code 且文案是英文 {@code {"message":"Internal error"}}，
 * 无法按 code 分支也无法定位线上 500；同时 404/405/415 等框架异常被 Exception 兜底吞成 500。
 */
...
public class GlobalExceptionHandler {
  ...
  /** 幂等键格式非法：此前抛 IllegalArgumentException 落到 Exception 兜底变成 500。 */
  @ExceptionHandler(IllegalArgumentException.class)
  public ResponseEntity<Map<String, Object>> handleIllegalArgument(IllegalArgumentException exception) {
    log.warn("Invalid media request argument: {}", exception.getMessage());
    return badRequest("INVALID_ARGUMENT", "请求参数不合法");
  }
  ...
  public ResponseEntity<Map<String, Object>> handleNotFound(Exception exception) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(body("ROUTE_NOT_FOUND", "接口不存在"));
  }
  ...
  public ResponseEntity<Map<String, Object>> handleMethodNotSupported(HttpRequestMethodNotSupportedException exception) {
    return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
        .body(body("METHOD_NOT_ALLOWED", "请求方法不被支持"));
  }
  ...
  public ResponseEntity<Map<String, Object>> handleGeneric(Exception exception, HttpServletRequest request) {
    log.error("Unhandled media-service exception, uri={}",
