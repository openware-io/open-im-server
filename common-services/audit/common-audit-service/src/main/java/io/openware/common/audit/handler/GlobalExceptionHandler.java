package io.openware.common.audit.handler;
...
/**
 * 审计服务统一异常出口：把业务异常与框架异常映射为稳定 JSON（{code, message}）。
 *
 * <p>此前本服务没有任何 {@code @RestControllerAdvice}，非业务异常（如对 {@code tenantId}
 * 做强制类型转换失败的 ClassCastException）会直接变成 Spring 默认错误体
...
public class GlobalExceptionHandler {
  ...
  /** 路径/查询参数类型不匹配（如 {id} 传了非数字）属于客户端错误，必须回 400。 */
  @ExceptionHandler(org.springframework.web.method.annotation.MethodArgumentTypeMismatchException.class)
  public ResponseEntity<Map<String, Object>> handleTypeMismatch(
      org.springframework.web.method.annotation.MethodArgumentTypeMismatchException exception) {
    return badRequest("INVALID_ARGUMENT", "参数类型不合法: " + exception.getName());
  }

  /** 参数绑定失败（缺参/格式错误）统一收敛为 400，不能被兜底吞成 500。 */
  ...
  public ResponseEntity<Map<String, Object>> handleGeneric(Exception exception, HttpServletRequest request) {
    log.error("Unhandled audit-service exception, uri={}",
