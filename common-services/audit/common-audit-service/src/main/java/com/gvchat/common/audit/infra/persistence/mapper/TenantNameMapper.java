...
import com.gvchat.common.audit.infra.persistence.row.TenantNameRow;
 ...
 *
 * <p>审计表已迁到独立 schema {@code gv_audit}（方案 docs/renovation/AUDIT_STORAGE_01_SERVICE.md），
 * 因此这里**必须显式限定 schema**（{@code gv_saas.tnt_tenant}）——不能再依赖连接默认库。
...
public interface TenantNameMapper {
    ...
    @Select("""
        <script>
        SELECT id, name FROM gv_saas.tnt_tenant
        WHERE id IN
