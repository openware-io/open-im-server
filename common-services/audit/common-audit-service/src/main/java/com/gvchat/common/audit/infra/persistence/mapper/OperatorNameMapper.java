...
import com.gvchat.common.audit.infra.persistence.row.OperatorNameRow;
 ...
 * {@code account_id} 与之一致）。审计表已迁到独立 schema {@code gv_audit}，因此这里**必须显式限定 schema**。
 *
...
public interface OperatorNameMapper {
    ...
    @Select("""
        ...
        FROM gv_saas.saa_admin_account
     ...
     */
    @Select("""
        SELECT platform_account_id FROM gv_saas.saa_admin_account
